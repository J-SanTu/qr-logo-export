...
The documentation states that normal API requests have no fixed request limit, while abusive, malicious, or illegal traffic may be rejected. The endpoint exposes no logo parameter and no multi-item request body. This skill therefore calls it once per unique URL and performs FP branding locally.
