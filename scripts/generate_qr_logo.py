...
HEX_COLOR_RE = re.compile(r"#?(?:[0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})\Z")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--color",
        dest="qr_color",
        help="QR module color as #RGB or #RRGGBB (default: dominant logo color)",
    )
...
def normalize_qr_color(value: str) -> str:
    candidate = value.strip()
    if not HEX_COLOR_RE.fullmatch(candidate):
        raise ValueError("QR color must be a hexadecimal value such as #000000 or #1A5FA9")
...
def render_logo_path_mask(
    ...
    for polygon in polygons:
        points = [(x * factor + offset_x, y * factor + offset_y) for x, y in polygon]
...
def render_logo(logo_asset: LogoAsset, pixel_size: int) -> Image.Image:
    ...
    for logo_path in logo_asset.paths:
        mask = render_logo_path_mask(
            logo_path, work_size, factor, offset_x, offset_y
        )
...
def dominant_logo_color(logo_asset: LogoAsset) -> str:
    declared_colors = list(
        dict.fromkeys(ImageColor.getrgb(path.fill)[:3] for path in logo_asset.paths)
    )
    rendered = render_logo(logo_asset, 512)
    weights = {color: 0 for color in declared_colors}
    nearest_colors: dict[tuple[int, int, int], tuple[int, int, int]] = {}
    pixels = (
    ...
    for red, green, blue, alpha in pixels:
        ...
        nearest = nearest_colors.get(pixel)
        if nearest is None:
            nearest = min(
                declared_colors,
                key=lambda color: sum(
                    (component - target) ** 2
                    for component, target in zip(pixel, color)
                ),
            )
            nearest_colors[pixel] = nearest
        weights[nearest] += alpha
    if not weights:
        raise ValueError("logo SVG has no visible pixels for automatic QR color extraction")
    red, green, blue = max(weights, key=weights.get)
    return f"#{red:02X}{green:02X}{blue:02X}"


def resolve_qr_color(value: str | None, logo_asset: LogoAsset) -> tuple[str, str]:
    if value is not None:
        return normalize_qr_color(value), "explicit"
    return dominant_logo_color(logo_asset), "logo-dominant"
...
def verify_png(path: Path, expected: str, timeout: float, retries: int) -> tuple[bool, list[str]]:
    boundary = f"----qrlogo{uuid.uuid4().hex}"
...
def write_manifests(
    ...
    manifest = {
        "specification": {
            "qr_color": qr_color,
            "qr_color_source": qr_color_source,
...
def main() -> int:
    ...
    try:
        ...
        qr_color, qr_color_source = resolve_qr_color(args.qr_color, logo)
    ...
    print(f"QR color: {qr_color} ({qr_color_source})", flush=True)
    results: list[Result] = []
    ...
    for position, item in enumerate(items, start=1):
        ...
        try:
            ...
            render_png(matrix_size, visible_modules, args.size, logo, qr_color).save(
